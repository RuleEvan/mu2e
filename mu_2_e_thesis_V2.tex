...
We express the final decay rate by their dependence on the muon lower component
\begin{equation}
\Gamma(\mu\rightarrow e)=\Gamma^{(0)}+2\frac{\braket{f}}{\braket{g}}\Gamma^{(1)}+\left(\frac{\braket{f}}{\braket{g}}\right)^2\Gamma^{(2)}
\end{equation}
The factor of two in the first-order corresponds stems from the fact that these are interference terms.

Defining the leptonic tensors
\begin{equation}
\begin{split}
