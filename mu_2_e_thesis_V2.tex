\documentclass{book}[letterpaper,12pt]
\usepackage{amsmath}
\usepackage{graphicx}
\usepackage{braket}
...
\end{split}
\end{equation}
The multipole projections defined in this section are valid for any local nuclear charge $J_0(\vec{x})$ or current $\vec{J}(\vec{x})$. In the next section, we will construct these projections for the two nuclear charges and three nuclear currents which appear in the effective theory of elastic $\mu\rightarrow e$ conversion.
...
\end{split}
\end{equation}
where the single-nucleon multipole operators which we obtain are those familiar from the study of semi-leptonic weak interactions \cite{DONNELLY1979103,SEROT1979408}. Adding a label to denote isospin, the single-nucleon response functions are
\begin{equation}
\begin{split}
...
Having successfully traded the quarks for nucleons, the one- and two-nucleon CLFV amplitudes are
\begin{equation}
\begin{split}
...
We now proceed to the calculation of the branching ratio, having replaced the two-nucleon current $J_\alpha^{(2)}$ with an effective contribution to $J_\alpha^{(1)}$ via the replacement in Eq. (\ref{eq:one_body_sub}).  Writing the standard muon capture rate as $\Gamma_\mathrm{capt}=\kappa_\mathrm{capt}m_{\mu}^5/v^4$, the branching ratio for scalar-mediated $\mu\rightarrow e$ conversion can be expressed simply as 
\begin{equation}
B(\mu\rightarrow e)=\left(\frac{v}{\Lambda}\right)^4\frac{1}{\kappa_\mathrm{capt}}\left(\left|\tau^{(+1)}\right|^2+\left|\tau^{(-1)}\right|^2\right),
\end{equation}
...
$C_{DR}=(r/8e)C_{SR}$ with $C_{SR}=C_{SR}^{(d)}=C_{SR}^{(s)}=C_{SR}^{(b)}$ with all other Wilson coefficients set to zero. R-parity conserving SUSY seesaw parameter space \cite{KITANO2003300} and within $R$-parity violating SUSY \cite{PhysRevD.56.100,HUITU1998355,FAESSLER200025}.
...
\label{eq:L_higgs}
\end{equation}
specified by two unknown Yukawa-like coupling parameters $Y_{e\mu}$, $Y_{\mu e}$. The tree-level Higgs exchange shown in Fig. \ref{fig:CLFV_higgs_diagrams} (a) induces the scalar quark couplings
\begin{equation}
\begin{split}
...
The Yukawa-like CLFV interaction generates a coupling of the leptons to on-shell photons at one- and two-loop level. The one-loop diagrams are shown in Fig. \ref{fig:CLFV_higgs_diagrams}. 
\begin{figure}
\centering
\subfloat[]{
...
\end{split}
\end{equation}
The leading one-loop diagram is suppressed by the muon's Yukawa coupling to the Higgs $Y_{\mu\mu}$. At two-loop order, this suppression can be avoided by the Bar-Zee diagrams shown in Fig. \ref{fig:CLFV_higgs_diagrams_2_loops}, whose flavor-conserving analogs contribute to fermion electric dipole moments \cite{PhysRevLett.65.21,Abe:2013qla}. We do not reproduce the full result for the two-loop amplitudes here---complete gauge-invariant expressions can be found in \cite{Cirigliano:2021img}. Numerically, the two-loop contributions to the dipole Wilson coefficients are roughly three orders of magnitude larger than the one-loop contributions
\begin{equation}
\begin{split}
...
The nucleon currents can be separated into isoscalar  and isovector components as
\begin{equation}
\begin{split}
