...
Chapter \ref{chap:two_nucleon} describes in greater detail the treatment of the two-nucleon operator that contributes to scalar-mediated coherent conversion at next-to-leading order. including the shell-model calculation.

In Chapter \ref{chap:conclusion}, we conclude with a summary of the progress reported in this thesis and discuss a few directions for future work. In particular, we describe an extension of the effective theory formalism to include the inelastic conversion process in which the nucleus transitions to an excited state. Second, we describe the work that remains in matching the nuclear-scale effective theory to higher-scale effective theories.
...
Appendix \ref{app:spinor_expansion}
...
\caption{Importance of the relativistic muon corrections compared to the leading-order result. $\braket{f}/\braket{g}$ is the ratio of monopole-averaged lower and upper muon wave functions, as defined in Eq. (\ref{eq:f_g_ratio}). In the remaining columns, the interference term $W_{OO^{(i)}}$ between upper- and lower-component nuclear responses is compared to the corresponding upper component response function $W_{O}$.}
...
\chapter{The Spherical Basis and Tensor Operators}
\label{app:tensors}
\thispagestyle{headings}
